...
void HtmlRead(string readFileName, string addr) {
	...
	while (!inFile.eof()) {
		...
		if (cntyh == 20 || inFile.eof()) {
			Read();
			TransGoto();
			for (int i = 0; i <= allcontentindex; i++) line += allcontent[i];
			for (int i = 0; i < line.length(); i++) {
				...
				if (tk == 1) {
					...
					if (ExTable(compareC)) { // 만들어진 단어, 예약어 검사
						...
						if (compareC == "document") {
							for (i= i+1; i < line.length(); i++) {
								a += line[i];
								if ((line[i] == ';') || (line[i] == 32) || (line[i] == '(') || (line[i] == '=')) break;
							}
						}
						continue;
					}
				...
				if (compareC == "onclick") {
					...
					for (i; i < line.length(); i++) {
						if (!((65 <= line[i] && line[i] <= 90) || (97 <= line[i] && line[i] <= 122) || (48 <= line[i] && line[i] <= 57) || (line[i] == '$') || (line[i] == '_'))) {
							a += salt[0] + sha256(transStr + salt).substr(0, 40);
							i = i - 1;
							break;
						}
						else {
...
string  Discrimination(string compare) {
	string check[] = { "for(" , "if(", "while(", "do{", "if (", "for (" , "do {", "while (" };
...
void Read() {
	...
	for (int i = 0; i < length; i++) {
		Disc = "";
		if (length - i > 6) {
			for (int k = 0; k < 7; k++) {  // 현재 for문만 체크함
		...
		if (cont != "") {
			...
			if (cont != "do{" && cont != "do {" && cont != "if(" && cont != "if (") {
				bool usedbrace = false;
				int bracket = 0;
				...
				for (int j = i; j < length; j++) {
					...
					if (CPST[j] == 40 && usedbracket == false)  bracket++;
					if (CPST[j] == 41 && usedbracket == false) {
						bracket--;
						if (bracket == 0) {
							for (int braceint = j + 1; braceint <= j + 6; braceint++) {
								if (CPST[braceint] == 123) {
									usedbrace = true;
									usedbracket = true;
									break;
								}
					...
					if (CPST[j] == 123 && usedbrace == true) {
						repeat++;
					}
					if (CPST[j] == 125 && usedbrace == true) {
						repeat--;
						if (repeat == 0) {
							allcontent[allcontentindex] = gototemp;
							contralkinds[allcontentindex] = cont;
					...
					if (CPST[j] == 125) {
						...
						if (repeat == 0) {
							...
							for (int k = j + 1; k <= j + 9; k++) {
								ifrept += CPST[k];
								if (ifrept == "else{" || ifrept == "else if(" || ifrept == "else {" || ifrept == "else if (") {
									break;
								}
							}
							if (ifrept == "else{" || ifrept == "else if(" || ifrept == "else {" || ifrept == "else if (") continue;
							allcontent[allcontentindex] = gototemp;
							contralkinds[allcontentindex] = cont;
							allcontentindex++;
							i = j;
...
void TransGoto() {
	for (int i = 0; i < allcontentindex; i++) {
		if (contralkinds[i] == "for(" || contralkinds[i] == "for (") {
			allcontent[i] = TransGotoFor(allcontent[i]);
		}
		else if (contralkinds[i] == "while(" || contralkinds[i] == "while (") {
			allcontent[i] = TransGotoWhile(allcontent[i]);
		}
		else if (contralkinds[i] == "do{" || contralkinds[i] == "do {") {
...
string TransGotoFor(string contant) {
	...
	for (int i = 0; i < intend; i++) {
		if (contant[i] == 40 && first1 == false) {
			...
			fortransgoto[transgotoindex] += countforstring + "ae__$" + to_string(Goto_index(index + 1)) + "();";
			fortransgoto[transgotoindex] = replace_all(fortransgoto[transgotoindex], "var", "let");
			index++;
			transgotoindex++;
			first1 = true;
		}
			...
			fora = "function " + countforstring + "ae__$" + to_string(Goto_index(index)) + "(){if(";
			...
			fora = ")return(" + countforstring + "ae__$" + to_string(Goto_index(index + 1)) + "());}";
			fortransgoto[transgotoindex] += fora;
				...
				if (contant[first] == 123) {
					...
					for (int i = 0; i < 3; i++) {
						if (i == 0 || i == 2) {
							...
							temp[i] = replace_all(temp[i], "continue;", "return(" + countforstring + "ae__$" + to_string(plusint) + "());");
						}
						cont += temp[i];
					}
					break;
			...
			fora = "function " + countforstring + "ae__$" + to_string(Goto_index(index)) + "(){";
			string foraa = "return(" + countforstring + "ae__$" + to_string(plusint) + "());}";
			fortransgoto[transgotoindex] += fora + cont + foraa;

			fora = "function " + countforstring + "ae__$" + to_string(plusint) + "(){";
			fortransgoto[transgotoindex] += fora + plus + ";";
			fora = "return(" + countforstring + "ae__$" + to_string(repeat) + "());}";
			fortransgoto[transgotoindex] += fora;
...
string TransGotoWhile(string contant) {
	...
	for (int i = 0; i < intend; i++) {
		if (contant[i] == 40) {
			overlap++;
			ifa = countwhilestring + "wh__$" + to_string(Goto_index(index)) + "(); " + "function " + countwhilestring + "wh__$" + to_string(Goto_index(index)) + "(){";
			...
			ifa = "return(" + countwhilestring + "wh__$" + to_string(Goto_index(index + 1)) + "());}";
			...
			overlap++;
			ifa = "function " + countwhilestring + "wh__$" + to_string(Goto_index(index)) + "(){";
			whiletransgoto[transgotoindex] += ifa;
			...
			for (int i = 0; i < 3; i++) {
				if (i == 0 || i == 2) {
					temp[i] = replace_all(temp[i], "break;", "return 0;");
					temp[i] = replace_all(temp[i], "continue;", "return(" + countwhilestring + "wh__$" + to_string(Goto_index(index - 1)) + "());");
				}
				plus += temp[i];
			}
			ifa = "return(" + countwhilestring + "wh__$" + to_string(Goto_index(index - 1)) + "());}";
...
string TransGotoDowhile(string contant) {
	...
	for (int i = 0; i < intend; i++) {
		if (contant[i] == 123) {
			overlap++;
			ifa = countdowhilestring + "dw__$" + to_string(Goto_index(index)) + "(); " + "function " + countdowhilestring + "dw__$" + to_string(Goto_index(index)) + "(){";
			...
			for (int i = 0; i < 3; i++) {
				if (i == 0 || i == 2) {
					temp[i] = replace_all(temp[i], "break;", "return 0;");
					temp[i] = replace_all(temp[i], "continue;", "return(" + countdowhilestring + "dw__$" + to_string(Goto_index(index + 1)) + "());");
				}
				plus += temp[i];
			}
			ifa = "return(" + countdowhilestring + "dw__$" + to_string(Goto_index(index + 1)) + "());}";
			...
			overlap++;
			ifa = "function " + countdowhilestring + "dw__$" + to_string(Goto_index(index)) + "(){";
